...
//  MusicVideo
//
//  Created by Gary Nothom on 4/3/17.
//  Copyright © 2017 LytxServices. All rights reserved.
//
    ...
    static func extractVideoDataFromJson(videoDataObject: AnyObject) -> [Video] {
        ...
        if let feeds = videoData["feed"] as? JSONDictionary, let entries = feeds["entry"] as? JSONArray {
            ...
            for (index, data) in entries.enumerated() {
                ...
                let currentVideo =
                    Video(vRank: index + 1, vName: vName, vImageUrl: vImageUrl, vVideoUrl: vVideoUrl, vRights: vRights, vPrice: vPrice, vArtist: vArtist, vImid: vImid, vGenre: vGenre, vLinkToiTunes: vLinkToiTunes, vReleaseDate: vReleaseDte)
